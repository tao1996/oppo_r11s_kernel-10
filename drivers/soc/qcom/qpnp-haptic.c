...
static int qpnp_hap_brake_config(struct qpnp_hap *hap, u8 *brake_pat)
{
	...
	u32 temp;
	u8 *pat_ptr, val;
#ifndef CONFIG_PRODUCT_OPPO //Fanhong.Kong@PSW.BSP.CHG,add 2017/12/01 for O vbrator c048 brake
	if (!hap->en_brake)
		return 0;
...
static void qpnp_hap_td_enable(struct timed_output_dev *dev, int time_ms)
{
	struct qpnp_hap *hap = container_of(dev, struct qpnp_hap,
					 timed_dev);
	int rc;
#ifdef CONFIG_PRODUCT_OPPO//Fanhong.Kong@PSW.BSP.CHG, 2018/1/20, Add for vib stopped by timer before				 
	bool state = !!time_ms;
	ktime_t rem;
#endif
...
#ifndef CONFIG_PRODUCT_OPPO//Fanhong.Kong@PSW.BSP.CHG, 2018/1/20, Add for vib stopped by timer before
	if (time_ms == 0) {
		/* disable haptics */
